...
export const useGameLogic = ({
  ...
  // Ref to hold the latest currentPath state
  const currentPathRef = useRef<L.LatLngExpression[]>([]);
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      watchId.current = navigator.geolocation.watchPosition(
        async (position) => {
          ...
          // Get the current path from the ref
          const prevPath = currentPathRef.current;
          const updatedPath = [...prevPath, newLocation];

          // Early exit if player is not alive
          if (!isPlayerAlive) {
            setCurrentPath([]); // Clear path synchronously
            await supabase.from('players').update({ current_path: [] }).eq('user_id', session?.user?.id);
            return;
          }
          ...
          if (updatedPath.length >= 3) {
            ...
            for (let i = 0; i < updatedPath.length - 3; i++) {
              ...
              if (turf.lineIntersect(newSegmentTurf, existingSegmentTurf).features.length > 0) {
                deathReason = 'You crossed your own path!';
                break;
              }
            }
          }

          // If already dead, no need for further checks
          if (deathReason) {
            handlePlayerDeath(deathReason);
            setCurrentPath([]); // Synchronous update
            await supabase.from('players').update({ current_path: [] }).eq('user_id', session?.user?.id);
            return;
          }
              ...
              if (ownTerritoryTurfCoords.length >= 4) {
                try {
                  ...
                  if (turf.lineIntersect(currentPathTurf, ownTerritoryPolygon).features.length > 0) {
                    deathReason = 'You crossed your own territory!';
                    break;
                  }
                } catch (e) {
          ...
          if (killedByPlayerTerritory) {
            deathReason = `You were killed by ${killedByPlayerTerritory.username}'s territory!`;
          }
          ...
          if (currentPathTurf && otherPlayers.length > 0) {
            ...
            if (killedByCrossingTerritory) {
              deathReason = `You crossed ${killedByCrossingTerritory.username}'s territory!`;
            }
          }
          ...
          setCurrentPath(updatedPath); // Synchronous update
        },
        (error) => {
